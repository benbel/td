...
\begin{document}
    ...
    effectivement réalisés, autrement dit \(Y_{\text{Trantor}}\).

    Soit \(T_{\text{Trantor}} = 1\) et il est possible d'observer
    \(Y_{\text{Trantor}} = Y_{\text{Trantor}}\left(1\right)\) mais pas
    \(Y_{\text{Trantor}}\left(0\right)\) ; soit
    \(T_{\text{Trantor}} = 0\) et il est possible d'observer
    \(Y_{Y_{\text{Trantor}} = \text{Trantor}}\left(0\right)\) mais pas
    ...
    Supposons par exemple qu'il y ait deux types de régions, des régions \(A\) pour lesquelles
    \(Y\left(0\right) = 5\%\) et \(Y\left(1\right) = 10\%\) et des régions \(B\) pour lesquelles
    \(Y\left(0\right) = 11\%\) et \(Y\left(1\right) = 16\%\). Pour les deux types de régions,
    \(\boldsymbol{\Delta} = 5\%\).

    Si toutes les régions traitées sont de type \(A\) et toutes les régions non traitées sont de type \(B\),
    \begin{align*}
        \boldsymbol{\Delta'} & = &
        \mathbf{E}\left[Y_{i} | T\left(i\right) = 1\right]  - \mathbf{E}\left[Y_{i} | T\left(i\right) = 0\right]\\
         & = &
        \mathbf{E}\left[Y_{i}\left(1\right) | i \in A\right] - \mathbf{E}\left[Y_{i}\left(0\right) | i \in B\right] \\
        & = & 10\% - 11\% \\
        & = & - 1\% \\
        & \neq & \boldsymbol{\Delta}
    \end{align*}

    Reprenons la décomposition précédente :
    \begin{align*}
    \boldsymbol{\Delta'} & = & \underbrace{\boldsymbol{\Delta}}_{\text{Effet du traitement}} + \underbrace{\mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 1\right] - \mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 0\right]}_{\text{Différence entre traités et non traités en l'absence de traitement}} \\
    &=& \boldsymbol{\Delta} + \mathbf{E}\left[Y_{i}\left(0\right) | i \in A\right] - \mathbf{E}\left[Y_{i}\left(0\right) | i \in B\right] \\
    &=& \boldsymbol{\Delta} + 5\% - 11\% \\
    &=& \boldsymbol{\Delta} - 6\% \\
    \end{align*}

    Notre comparaison traités --- non traités ne permet pas de retrouver le véritable effet du traitement \(\boldsymbol{\Delta}\),
    puisque la différence entre traités et non traités en l'absence de traitement est non-nulle et inconnue.
    ...
    Si le traitement est assigné aléatoirement, il n'y a pas, en moyenne, de
    différence entre le groupe traité et le groupe non traité (dit groupe
    témoin).

    Si le traitement est assigné aléatoirement, cela implique que la probabilité
    d'être traitré est la même pour toutes les observations dans l'échantillon, et
    ce quelque soient leurs charactéristiques : autrement dit, \( \forall x,
    p\left(T|x\right) = p\left(T\right)\).

    Intuitivement, cela implique que
    la proportion de chaque sous-groupe sera la même dans l'échantillon entier,
    parmi les traités et parmi les non traités.

    Supposons par exemple que la
    probabilité d'être traité soit de 25\%, et pour reprendre notre exemple
    précédent, qu'il y ait 40 régions de type \(A\) et 60 de régions de
    type \(B\). Il y aura donc \(25\% \times 40 = 10\) régions de type \(A\)
    traitées,  \(\left( 1 - 25\%\right) \times 40 = 30\) régions de type \(A\)
    non traitées ; \(25\% \times 60 = 15\) régions de type \(B\)
    traitées,  \(\left( 1 - 25\%\right) \times 60 = 45\) régions de type \(B\)
    non traitées.

    La proportion de régions \(A\) parmi les traités est donc de
    \(\frac{10}{10+15} = 40\%\) et la proportion de régions \(B\) parmi les
    traités est de \(\frac{15}{10+15} = 60\%\).

    La proportion de régions \(A\) parmi les non traités est donc de
    \(\frac{30}{30+45} = 40\%\) et la proportion de régions \(B\) parmi les
    traités est de \(\frac{45}{30+45} = 60\%\).

    Les deux groupes, non traités et traités, ont donc la même composition, et
    donc la même moyenne de \(Y\left(0\right)\) et de \(Y\left(1\right)\)).

    On a :
    \[\mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 1\right] =
    Y_{0, A} \times p\left(A | T = 1)\right) + Y_{0, B} \times p\left(B | T = 1)\right) =
    5\% \times 40\% + 11\% \times 60\% = 8,6\%
    \]

    et :
    \[\mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 0\right] =
    Y_{0, A} \times p\left(A | T = 0)\right) + Y_{0, B} \times p\left(B | T = 0)\right) =
    5\% \times 40\% + 11\% \times 60\% = 8,6\%
    \]

    C'est à dire
    \[\mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 1\right] -
    \mathbf{E}\left[Y_{i}\left(0\right) | T\left(i\right) = 0\right] = 0\]

    Les essais contrôlé aléatoires ou \textit{randomized controlled trial} (\textsc{rct})
    permettent donc d'\og{}équilibrer\fg{} le groupe traité et le groupe témoin
    sur l'ensemble de leurs caractéristiques, observées ou non observées.
