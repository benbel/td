...
\begin{document}
    ...
    \subsection{} Quel est le niveau de consommation choisi par chaque individu ?
    ...
    \subsection{} Quelle est la quantité de travail demandée par les entreprises
    pour maximiser leur profit ?

    \subsection{} Représentez graphiquement les courbes d'offre et de demande de
    travail.

    \subsection{} Quel est le salaire d'équilibre \(s^{*}\) et la quantité de
    travail d'équilibre \(q^{*}\) sur le marché du travail ?

    \subsection{} Comment le salaire d'équilibre évolue-t-il avec le nombre \(N\)
    ...
    \subsection{} À l'aide de votre graphique, déterminez l'effet de cette
    ...
    \subsection{} Si l'entreprise veut acheter \(T^{m}\) unités de travail, quel
    salaire \(s^{m}\) doit-elle payer pour que les individus offrent cette
    quantité de travail ?

    \subsection{} Déduisez-en le profit de l'entreprise en fonction de
    \(T^{m}\). Quelle est la quantité de travail \(T^{m*}\) demandée par
    l'entreprise qui maximise son profit ? Quelle est la valeur de \(s^{m*}\) ?

    \subsection{} Comparez \(s^{m*}\) à \(s^{*}\). Lequel est le plus élevé et
    pourquoi ?

    \subsection{} Quel serait l'effet d'introduire un salaire minimum compris
    ...
    \subsection{} De quelle politique publique les auteurs souhaitent-ils mesurer
    les effets ? Quels en sont les effets attendus ?

    \subsection{} Expliquez en quoi consiste la méthode des \og différences de
    différences \fg{} utilisée par les auteurs. Sur quelles hypothèses
    repose-t-elle ? Sont-elles vérifiées ?

    \subsection{} Que concluent les auteurs sur l'effet de cette politique ?

    \section{Correction}

    \subsection{Concurrence pure et parfaite}
    ...
    est :
    \begin{itemize}
        \item Soit le salaire d'équilibre en l'absence de salaire minimum aurait été
        inférieur à \(s_{min}\), et dans ce cas le salaire minimum contraint les entreprises à payer
        au moins \(s_{min}\). Il faut donc résoudre le modèle pour la situation
        où \(s_{eq} = s_{min}\).
        \item Soit le salaire d'équilibre en l'absence de salaire minimum aurait été
        supérieur à \(s_{min}\), et dans ce cas le salaire minimum ne contraint pas les entreprises à payer
