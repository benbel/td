...
\begin{document}
    ...
    Quel est le niveau de consommation choisi par chaque individu ?
    ...
    Quelle est la quantité de travail demandée par les entreprises
    pour maximiser leur profit ?

    Représentez graphiquement les courbes d'offre et de demande de
    travail.

    Quel est le salaire d'équilibre \(s^{*}\) et la quantité de
    travail d'équilibre \(q^{*}\) sur le marché du travail ?

    Comment le salaire d'équilibre évolue-t-il avec le nombre \(N\)
    ...
    À l'aide de votre graphique, déterminez l'effet de cette
    ...
    Si l'entreprise veut acheter \(T^{m}\) unités de travail, quel
    salaire \(s^{m}\) doit-elle payer pour que les individus offrent cette
    quantité de travail ?

    Déduisez-en le profit de l'entreprise en fonction de
    \(T^{m}\). Quelle est la quantité de travail \(T^{m*}\) demandée par
    l'entreprise qui maximise son profit ? Quelle est la valeur de \(s^{m*}\) ?

    Comparez \(s^{m*}\) à \(s^{*}\). Lequel est le plus élevé et
    pourquoi ?

    Quel serait l'effet d'introduire un salaire minimum compris
    entre \(s^{*}\) et \(s^{m*}\) sur la quantité de travail échangée à
    l'équilibre ?

    \subsection{Card \& Krueger, 1994}
    ...
    De quelle politique publique les auteurs souhaitent-ils mesurer
    les effets ? Quels en sont les effets attendus ?

    Expliquez en quoi consiste la méthode des \og différences de
    différences \fg{} utilisée par les auteurs. Sur quelles hypothèses
    repose-t-elle ? Sont-elles vérifiées ?

    Que concluent les auteurs sur l'effet de cette politique ?
    ...
    lorsqu'elle maximise son profit, elle prend en compte l'effet de ses actions sur
    les prix.
